...
// [FIX] Changed ID to 21 to avoid collision with CMD_ADMIN_REGEN_KEYS (20)
...
static void add_pending_bot(hub_state_t *state, const char *uuid, const char *ip) {
    for (int i = 0; i < state->pending_count; i++) {
        if (strcmp(state->pending[i].uuid, uuid) == 0) {
            ...
            snprintf(state->pending[i].ip, sizeof(state->pending[i].ip), "%.255s", ip);
            return;
        }
    }
    int idx;
    ...
    snprintf(p->uuid, sizeof(p->uuid), "%.63s", uuid);
    snprintf(p->ip, sizeof(p->ip), "%.255s", ip);
...
static void broadcast_new_key(hub_state_t *state, const char *new_pub_key) {
    ...
    for (int i = 0; i < state->client_count; i++) {
        ...
        if (c->authenticated && c->type == CLIENT_BOT) {
            int enc_len = aes_gcm_encrypt(plain, total_plain, c->session_key, buffer + 4, tag);
            if (enc_len > 0) {
                ...
                if (write(c->fd, buffer, 4 + packet_len) == (4 + packet_len)) sent_count++;
            }
        }
    }
...
static void hub_state_add_bot_memory(hub_state_t *state, const char *uuid, const char *nick, const char *pub_key) {
...
void hub_broadcast_mesh_state(hub_state_t *state) {
    ...
    int w = snprintf(payload + offset, MAX_BUFFER - offset, "127.0.0.1:%d|", state->port);
    if (w > 0) offset += w;
    ...
    for (int i = 0; i < state->peer_count; i++) {
        ...
        for (int c = 0; c < state->client_count; c++) {
            if (state->clients[c]->type == CLIENT_HUB && state->clients[c]->authenticated) {
                if (state->peers[i].fd == state->clients[c]->fd && state->peers[i].fd > 0) {
                    is_up = 1;
                    break;
                }
            }
        }
        int written = snprintf(payload + offset, MAX_BUFFER - offset, "%.255s:%d:%d,", 
                             state->peers[i].ip, state->peers[i].port, is_up);
        if (written > 0 && written < (MAX_BUFFER - offset)) offset += written;
    }
    
    if (offset < MAX_BUFFER - 1) payload[offset++] = ';';
    payload[offset] = 0;
    ...
    for(int i=0; i<state->peer_count; i++) if(state->peers[i].connected) connected_peers++;

    int active_bots = 0;
    for(int i=0; i<state->client_count; i++) {
        if(state->clients[i]->type == CLIENT_BOT && state->clients[i]->authenticated) active_bots++;
    }
    ...
    snprintf(final_packet, sizeof(final_packet), "%d:%d:%d|%.4096s", 
             connected_peers, state->peer_count, active_bots, payload);

    int payload_len = strlen(final_packet);
    if (payload_len > MAX_BUFFER - 100) payload_len = MAX_BUFFER - 100;
    ...
    if (sscanf(payload, "%d:%d:%d", &remote_conn, &remote_total, &remote_bots) >= 2) {
        for (int i = 0; i < state->peer_count; i++) {
            if (state->peers[i].connected && state->peers[i].fd == c->fd) {
                ...
                snprintf(state->peers[i].last_gossip, sizeof(state->peers[i].last_gossip), "%.1023s", payload);
                return;
            }
        }
    }
...
void hub_generate_sync_packet(hub_state_t *state, char *buffer, int max_len) {
    ...
    for (int i = 0; i < state->bot_count; i++) {
        ...
        // ADDED 'b|' prefix to match standard sync logic
        int written = snprintf(buffer + offset, max_len - offset, "b|%s|t||%ld\n", b->uuid, (long)b->last_sync_time);
...
static void process_peer_sync(hub_state_t *state, char *payload, int origin_fd) {
    ...
    while(line) {
        // [FIX 1] Check for the 'b|' prefix and skip it
        char *ptr = line;
            ...
            *p1 = 0;
            ...
            if (p2) {
                *p2 = 0;
                ...
                if (p3) {
                    *p3 = 0;
                    snprintf(uuid, sizeof(uuid), "%.63s", ptr);
                    snprintf(key, sizeof(key), "%.31s", p1 + 1);
                    snprintf(val, sizeof(val), "%s", p2 + 1);
                    ts = atol(p3 + 1);

                    if (hub_storage_update_entry(state, uuid, key, val, ts)) {
                        updates++;
                        // [FIX 2] Ensure we forward with the 'b|' prefix so peers can parse it
                        if (sizeof(forward_buf) - fwd_offset > 1200) { // Safety margin for max val + metadata
                             int w = snprintf(forward_buf + fwd_offset, sizeof(forward_buf) - fwd_offset,
                                            "b|%s|%s|%s|%ld\n", uuid, key, val, ts);
                             if (w > 0 && w < (int)(sizeof(forward_buf) - fwd_offset)) fwd_offset += w;
                        }
                    }
                }
            }
    ...
    if (updates > 0) {
        ...
        // Only broadcast if we have data to forward
        if (fwd_offset > 0) {
...
static bool send_response(hub_state_t *state, hub_client_t *client, const char *msg) {
    ...
    unsigned char tag[GCM_TAG_LEN];
    int len = strlen(msg);
    int enc_len = aes_gcm_encrypt((unsigned char*)msg, len, client->session_key, buffer + 4, tag);
    if (enc_len > 0) {
        memcpy(buffer + 4 + enc_len, tag, GCM_TAG_LEN);
        uint32_t net_len = htonl(enc_len + GCM_TAG_LEN);
        memcpy(buffer, &net_len, 4);
        if (write(client->fd, buffer, 4 + enc_len + GCM_TAG_LEN) != (4 + enc_len + GCM_TAG_LEN)) {
...
static bool send_pong(hub_state_t *state, hub_client_t *c) {
    ...
    if (enc_len > 0) {
        ...
        uint32_t net_len = htonl(enc_len + GCM_TAG_LEN);
        memcpy(buffer, &net_len, 4);
        if (write(c->fd, buffer, 4 + enc_len + GCM_TAG_LEN) != (4 + enc_len + GCM_TAG_LEN)) {
...
static bool handle_admin_command(hub_state_t *state, hub_client_t *client, int cmd, char *payload) {
    ...
    switch(cmd) {
        ...
        case CMD_ADMIN_GET_PENDING:
            {
                int offset = 0;
                if (state->pending_count == 0) strcpy(response, "No pending bots.");
                else {
                    offset += snprintf(response, sizeof(response), "--- Pending Authorization ---\n");
                    for (int i = 0; i < state->pending_count; i++) {
                        struct tm *t = localtime(&state->pending[i].last_attempt);
                        char tbuf[64];
                        strftime(tbuf, sizeof(tbuf), "%H:%M:%S", t);
                        offset += snprintf(response + offset, sizeof(response) - offset, "[%d] %s | IP: %s\n", i+1, state->pending[i].uuid, state->pending[i].ip);
                    }
                }
                return send_response(state, client, response);
            }
        case CMD_ADMIN_APPROVE:
            if (payload && strlen(payload) > 0) {
                ...
                if (strlen(payload) < 4) {
                    ...
                    if (idx > 0 && idx <= state->pending_count) snprintf(target_uuid, sizeof(target_uuid), "%.63s", state->pending[idx-1].uuid);
                    else return send_response(state, client, "ERROR: Invalid Index.");
                } else snprintf(target_uuid, sizeof(target_uuid), "%.63s", payload);
                if (target_uuid[0]) {
                    time_t now = time(NULL);
        ...
        case CMD_ADMIN_ADD_PEER:
            if (payload && strlen(payload) > 0) {
                ...
                if (sscanf(payload, "%255[^:]:%d", ip, &port) == 2) {
                    if (state->peer_count < MAX_PEERS) {
                        snprintf(state->peers[state->peer_count].ip, sizeof(state->peers[state->peer_count].ip), "%.63s", ip);
            ...
            {
                int loff = 0;
                loff += snprintf(response + loff, sizeof(response)-loff, " --- Current Local Peers ---\n");
                for (int i = 0; i < state->peer_count; i++) {
                    loff += snprintf(response + loff, sizeof(response)-loff, "[%d] %s:%d\n", i + 2, state->peers[i].ip, state->peers[i].port);
                }
                loff += snprintf(response + loff, sizeof(response)-loff, "Usage: ADD_PEER IP:PORT\n");
                return send_response(state, client, response);
            }
        case CMD_ADMIN_LIST_PEERS:
            {
                ...
                for(int k=0; k<line_len; k++) { response[offset++] = '-'; }
                response[offset++] = '\n';
                offset += snprintf(response + offset, sizeof(response) - offset, " %-*s |", peer_col_width, "Peer");
                for(int i=0; i<count; i++) offset += snprintf(response + offset, sizeof(response) - offset, " %-2d |", i+1);
                offset += snprintf(response + offset, sizeof(response) - offset, " Mesh State   | Bots |\n");
                for(int k=0; k<line_len; k++) { response[offset++] = '-'; }
                response[offset++] = '\n';
                ...
                for (int row = 0; row < count; row++) {
                    char peer_str[512]; snprintf(peer_str, 512, "%.255s:%d", all_peers[row].ip, all_peers[row].port);
                    offset += snprintf(response + offset, sizeof(response) - offset, " %d. %-*s |", row+1, peer_col_width - 3, peer_str);
                    ...
                    for (int col = 0; col < count; col++) {
                        ...
                        offset += snprintf(response + offset, sizeof(response) - offset, " %s |", cell);
                    }
                    bool is_offline = false;
                    if(row_total > 0) {
                        if(row_connected > 0) offset += snprintf(response + offset, sizeof(response) - offset, " %d/%d Connected |", row_connected, row_total);
                        else { offset += snprintf(response + offset, sizeof(response) - offset, " \033[31mOffline\033[0m       |"); is_offline = true; issues++; }
                    } else {
                        if (all_peers[row].is_me) offset += snprintf(response + offset, sizeof(response) - offset, " ---          |");
                        else { offset += snprintf(response + offset, sizeof(response) - offset, " \033[31mOffline\033[0m       |"); is_offline = true; issues++; }
                    }
                    if (is_offline) offset += snprintf(response + offset, sizeof(response) - offset, " ??   |\n");
                    else {
                        int bot_cnt = 0;
                        if (all_peers[row].is_me) {
                            for(int k=0; k<state->client_count; k++) if(state->clients[k]->type == CLIENT_BOT && state->clients[k]->authenticated) bot_cnt++;
                        } else {
                            for(int p=0; p<state->peer_count; p++) {
                                if(state->peers[p].connected && state->peers[p].port == all_peers[row].port && strcmp(state->peers[p].ip, all_peers[row].ip) == 0) {
                                    int rc, rt, rb; if(sscanf(state->peers[p].last_gossip, "%d:%d:%d|", &rc, &rt, &rb) == 3) bot_cnt = rb;
                                    break;
                                }
                            }
                        }
                        offset += snprintf(response + offset, sizeof(response) - offset, " %-4d |\n", bot_cnt);
                    }
                    if(all_peers[row].is_me) {
                         for(int p=0; p<state->peer_count; p++) {
                             bool active = false;
                ...
                for(int k=0; k<line_len; k++) { response[offset++] = '-'; } response[offset++] = '\n';
                char status_str[128]; 
                if (issues == 0 && state->peer_count > 0) snprintf(status_str, 64, "\033[32mHEALTHY\033[0m");
                else snprintf(status_str, 64, "\033[33mDEGRADED (%d ISSUES)\033[0m", issues);
                offset += snprintf(response + offset, sizeof(response) - offset, " [i] MESH STATUS: %s\n [Legend: -- = Self, UP = Connected, DN = Down, ?? = Unknown/Not Configured]\n", status_str);
                if (issues > 0) offset += snprintf(response + offset, sizeof(response) - offset, " --- Mesh Diagnostics ---\n%s", issue_log);
                return send_response(state, client, response);
            }
        case CMD_ADMIN_DEL_PEER:
            if (payload && strlen(payload) > 0) {
                int idx = atoi(payload);
                if (idx == 1) return send_response(state, client, "ERROR: Cannot remove local hub (Index 1).");
                if (idx > 1 && idx <= state->peer_count + 1) {
                    int target = idx - 2; 
                    if (state->peers[target].fd != -1) {
                        int target_fd = state->peers[target].fd;
                        for(int c=0; c<state->client_count; c++) {
                            if (state->clients[c]->fd == target_fd) { hub_disconnect_client(state, state->clients[c]); break; }
                        }
                    }
                    char confirm_msg[256];
                    snprintf(confirm_msg, sizeof(confirm_msg), "SUCCESS: Deleted Peer %s:%d.", state->peers[target].ip, state->peers[target].port);
                    for (int j = target; j < state->peer_count - 1; j++) state->peers[j] = state->peers[j+1];
                    state->peer_count--;
                    hub_config_write(state);
            ...
            {
                int loff = 0;
                loff += snprintf(response + loff, sizeof(response)-loff, " --- Remove Local Peer ---\n");
                for (int i = 0; i < state->peer_count; i++) {
                    loff += snprintf(response + loff, sizeof(response)-loff, "[%d] %s:%d\n", i + 2, state->peers[i].ip, state->peers[i].port);
                }
                loff += snprintf(response + loff, sizeof(response)-loff, "Enter Index to Remove: ");
                return send_response(state, client, response);
            }
        case CMD_ADMIN_GET_PUBKEY:
            {
                ...
                if (!pub && state->priv_key) {
                    BIO *bio = BIO_new(BIO_s_mem()); PEM_write_bio_RSA_PUBKEY(bio, state->priv_key);
                    int len = BIO_pending(bio); char *pem = malloc(len + 1); BIO_read(bio, pem, len); pem[len] = 0; BIO_free(bio);
                    state->public_key_pem = pem; pub = pem;
                }
        ...
        case CMD_ADMIN_SET_PRIVKEY:
            if (payload && strlen(payload) > 10) {
                RSA *new_rsa = load_private_key_from_memory(payload);
                if (new_rsa) {
                    ...
                    if (state->priv_key) RSA_free(state->priv_key);
                    state->private_key_pem = strdup(payload); state->priv_key = new_rsa;
                    hub_config_write(state);
        ...
        case CMD_ADMIN_GET_PRIVKEY:
            if (state->private_key_pem) return send_response(state, client, state->private_key_pem);
        ...
        case CMD_ADMIN_REGEN_KEYS:
             {
                 ...
                 if (hub_crypto_generate_keypair(&priv, &pub)) {
                     ...
                     if (state->public_key_pem) free(state->public_key_pem);
                     if (state->priv_key) RSA_free(state->priv_key);
                     state->private_key_pem = priv; state->public_key_pem = pub;
                     state->priv_key = load_private_key_from_memory(priv);
                     hub_config_write(state);
                     time_t now = time(NULL); struct tm *t = localtime(&now);
                     char f[64]; strftime(f, sizeof(f), "%Y%m%d%H%M_pub.pem", t);
                     FILE *fp = fopen(f, "w"); if (fp) { fputs(pub, fp); fclose(fp); }
                     return send_response(state, client, pub);
                 }
                 return send_response(state, client, "ERROR: Gen failed.");
             }
        case CMD_ADMIN_CREATE_BOT:
            {
                char nick[64];
                if (payload && strlen(payload) > 0) snprintf(nick, sizeof(nick), "%.63s", payload);
                else strcpy(nick, "UnnamedBot");
                ...
                if (hub_crypto_generate_bot_creds(&uuid, &priv_key, &pub_key)) {
                    ...
                    // Encrypt & Save
                    hub_config_write(state);
                    // Respond: SUCCESS|UUID|PRIVKEY
                    int w = snprintf(response, sizeof(response), "SUCCESS|%s|%s", uuid, priv_key);
                    if (w >= (int)sizeof(response)) send_response(state, client, "ERROR|Key too large.");
                    else send_response(state, client, response);

                    if (priv_key) { memset(priv_key, 0, strlen(priv_key)); free(priv_key); }
                    if (pub_key) free(pub_key);
                    if (uuid) free(uuid);
                } else send_response(state, client, "ERROR|Crypto generation failed.");
            }
            return true;
        default: return send_response(state, client, "ERROR: Unknown.");
    }
    return true;
}
...
static void process_bot_command(hub_state_t *state, hub_client_t *client, int cmd, char *payload) {
    switch(cmd) {
        case CMD_PING: hub_log("[HUB] Bot %s PING\n", client->id); break;
        case CMD_CONFIG_PUSH:
            hub_log("[HUB] Sync from %s\n", client->id);
            if (payload) {
                char target_nick[32], servers[256], chans[256], pass[128];
                time_t now = time(NULL);
                char sync_packet[MAX_BUFFER]; int sp_len = 0;
                hub_storage_update_entry(state, client->id, "t", "", now);
                sp_len += snprintf(sync_packet + sp_len, sizeof(sync_packet) - sp_len, "%s|t||%ld\n", client->id, (long)now);
                if (sscanf(payload, "%31[^|]|%255[^|]|%255[^|]|%127s", target_nick, servers, chans, pass) >= 1) {
                    hub_storage_update_entry(state, client->id, "n", target_nick, now);
                    sp_len += snprintf(sync_packet + sp_len, sizeof(sync_packet) - sp_len, "%s|n|%s|%ld\n", client->id, target_nick, (long)now);
                    hub_storage_update_entry(state, client->id, "b", pass, now);
                    sp_len += snprintf(sync_packet + sp_len, sizeof(sync_packet) - sp_len, "%s|b|%s|%ld\n", client->id, pass, (long)now);
                    char *sp, *t = strtok_r(servers, ",", &sp);
                    while(t) {
                        hub_storage_update_entry(state, client->id, "s", t, now);
                        sp_len += snprintf(sync_packet + sp_len, sizeof(sync_packet) - sp_len, "%s|s|%s|%ld\n", client->id, t, (long)now);
                        t = strtok_r(NULL, ",", &sp);
                    }
                    t = strtok_r(chans, ",", &sp);
                    while(t) {
                        hub_storage_update_entry(state, client->id, "c", t, now);
                        sp_len += snprintf(sync_packet + sp_len, sizeof(sync_packet) - sp_len, "%s|c|%s|%ld\n", client->id, t, (long)now);
                        t = strtok_r(NULL, ",", &sp);
                    }
                    hub_config_write(state);
...
bool hub_handle_client_data(hub_state_t *state, hub_client_t *client) {
    while (client->recv_len >= 4) {
        uint32_t net_len; memcpy(&net_len, client->recv_buf, 4);
        int packet_len = ntohl(net_len);
        if (packet_len > (MAX_BUFFER - 4) || packet_len <= 0) { hub_disconnect_client(state, client); return false; }
        if (client->recv_len < (4 + packet_len)) return true;
        unsigned char *data = client->recv_buf + 4;
        if (!client->authenticated) {
            unsigned char dec[512];
            int dec_len = rsa_private_decrypt(state->priv_key, data, packet_len, dec);
            if (dec_len > 32) {
                memcpy(client->session_key, dec, 32);
                char *payload = (char*)dec + 32; dec[dec_len] = 0;
                if (strncmp(payload, "ADMIN", 5) == 0) {
                    if (strcmp(payload + 6, state->admin_password) == 0) {
                        client->type = CLIENT_ADMIN; client->authenticated = true; snprintf(client->id, sizeof(client->id), "ADMIN");
                        hub_log("[HUB] Admin Login: %s\n", client->ip);
                    } else { hub_disconnect_client(state, client); return false; }
                } else if (strncmp(payload, "HUB", 3) == 0) {
                    char pass[128]; int claimed_port = 0;
                    int args = sscanf(payload + 4, "%127s %d", pass, &claimed_port);
                    if (args >= 1 && strcmp(pass, state->admin_password) == 0) {
                        client->type = CLIENT_HUB; client->authenticated = true; snprintf(client->id, sizeof(client->id), "HUB-PEER");
                        bool is_authorized_peer = false;
                        for(int p=0; p<state->peer_count; p++) {
                            bool ip_match = (strcmp(state->peers[p].ip, client->ip) == 0 || strcmp("127.0.0.1", client->ip) == 0);
                            if (ip_match) {
                                if (claimed_port > 0) { if (state->peers[p].port == claimed_port) { state->peers[p].connected = true; state->peers[p].fd = client->fd; is_authorized_peer = true; }
                                } else { state->peers[p].connected = true; state->peers[p].fd = client->fd; is_authorized_peer = true; }
                            }
                        }
                        if (!is_authorized_peer) { hub_disconnect_client(state, client); return false; }
                        ...
                        int payload_len = strlen((char*)plain + 5); int total_plain = 1 + 4 + payload_len;
                        memcpy(&plain[1], &payload_len, 4);
                        int cipher_len = aes_gcm_encrypt(plain, total_plain, client->session_key, buffer + 4, tag);
                        memcpy(buffer + 4 + cipher_len, tag, GCM_TAG_LEN);
                        int pkt_len = cipher_len + GCM_TAG_LEN; uint32_t nl = htonl(pkt_len); memcpy(buffer, &nl, 4);
                        if (write(client->fd, buffer, 4 + pkt_len) <= 0) hub_log("[MESH] Failed to send reply sync\n");
                    } else { hub_disconnect_client(state, client); return false; }
                } else {
                    bool auth = false;
                    for(int i=0; i<state->bot_count; i++) if (strcmp(state->bots[i].uuid, payload) == 0) { auth = true; break; }
                    if (auth) {
                        client->type = CLIENT_BOT; client->authenticated = true; snprintf(client->id, sizeof(client->id), "%.63s", payload);
                        hub_log("[HUB] Bot Login: %s\n", client->id);
                    } else {
                        hub_log("[HUB] Unauthorized: %s\n", payload); add_pending_bot(state, payload, client->ip); hub_disconnect_client(state, client); return false;
                    }
                }
            } else { hub_disconnect_client(state, client); return false; }
        } else {
            if (packet_len > GCM_TAG_LEN) {
                ...
                int pl = aes_gcm_decrypt(data, packet_len - GCM_TAG_LEN, client->session_key, plain, tag);
                if (pl > 0) {
                    if (plain[0] == CMD_PING) { if (!send_pong(state, client)) return false; }
                    else {
                        plain[pl] = 0;
                        if (client->type == CLIENT_ADMIN) { if (!handle_admin_command(state, client, plain[0], (char*)plain+5)) return false; }
                        else if (client->type == CLIENT_BOT) process_bot_command(state, client, plain[0], (char*)plain+5);
                        else if (client->type == CLIENT_HUB) {
                             if (plain[0] == CMD_PEER_SYNC) process_peer_sync(state, (char*)plain+5, client->fd);
                             else if (plain[0] == CMD_MESH_STATE) process_mesh_state(state, client, (char*)plain+5);
                        }
                    }
                }
            }
        }
        int consumed = 4 + packet_len; int remaining = client->recv_len - consumed;
        if (remaining > 0) memmove(client->recv_buf, client->recv_buf + consumed, remaining);
